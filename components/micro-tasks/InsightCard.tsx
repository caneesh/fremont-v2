...
interface InsightCardProps {
  ...
  onSwitchToReadingMode?: () => void  // Switch to passive hint ladder mode
  attempts: number
...
export default function InsightCard({
  ...
  return (
    ...
    >
      ...
      {/* "I'm stuck" fallback - switches to reading/passive mode */}
      {!isCompleted && onSwitchToReadingMode && (
        <div className="px-4 pb-4 border-t border-slate-200 dark:border-slate-700 pt-3 mt-2">
          <button
            onClick={onSwitchToReadingMode}
            className="w-full py-2 text-sm text-slate-400 dark:text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors flex items-center justify-center gap-2"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
            </svg>
            I&apos;m stuck, switch to reading mode
          </button>
        </div>
      )}
    </div>
