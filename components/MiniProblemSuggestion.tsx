...
export default function MiniProblemSuggestion({
  ...
  if (isLoading) {
    return (
      <div className={`${compact ? 'p-3' : 'p-6'} bg-amber-50 border border-amber-200 rounded-lg`}>
        <div className="flex items-center gap-2">
          <div className="inline-block animate-spin rounded-full h-4 w-4 border-b-2 border-amber-600" />
          <span className="text-sm text-amber-700">Finding practice problems...</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="bg-amber-50 border-2 border-amber-300 rounded-lg p-6">
      ...
      {/* Weak concepts */}
      <div className="mb-4">
        <p className="text-xs text-amber-700 mb-2 font-medium uppercase tracking-wide">
          Areas to strengthen:
        </p>
        <div className="flex flex-wrap gap-2">
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                ...
                <p className="text-xs text-amber-600 mt-1 italic">{mp.reason}</p>
              </div>
              <button
                onClick={() => onSelectProblem?.(mp)}
